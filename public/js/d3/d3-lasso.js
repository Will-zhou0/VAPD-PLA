...
return d[d.length-1];};return ", funcName].join("")
    ...
    function lasso() {
        ...
        function lasso(_this) {
            ...
            var g = _this.selectAll("g.lasso").data([1]).join("g")
                .attr("class", "lasso");

            // add the drawn path for the lasso
            var dyn_path = g.selectAll("path.drawn").data([1]).join("path")
                .attr("class", "drawn");

            // add a closed path
            var close_path = g.selectAll("path.loop_close").data([1]).join("path")
                .attr("class", "loop_close");

            // add an origin node
            var origin_node = g.selectAll("circle.origin").data([1]).join("circle")
                .attr("class", "origin");
