...
export default {
  ...
  data() {
    let width = 490, height = 230, margin = 10;
    return {
  ...
  methods: {
    ...
    initData() {
      ...
      that.zoomStr = "M919.264 905.984l-138.912-138.912C851.808 692.32 896 591.328 896 480c0-229.376-186.624-416-416-416S64 250.624 64 480s186.624 416 416 416c95.008 0 182.432-32.384 252.544-86.208l141.44 141.44a31.904 31.904 0 0 0 45.248 0 32 32 0 0 0 0.032-45.248zM128 480C128 285.92 285.92 128 480 128s352 157.92 352 352-157.92 352-352 352S128 674.08 128 480z M625.792 448H512v-112a32 32 0 0 0-64 0V448h-112a32 32 0 0 0 0 64H448v112a32 32 0 1 0 64 0V512h113.792a32 32 0 1 0 0-64z"
      that.initRequire()
    },
    // 数据请求
    async initRequire() {
    ...
    drawChart() {
      ...
      that.svg = d3.select("." + that.barClass)
        .selectAll("svg." + that.barClass + "timeLineSvg").data([1]).join("svg")
      ...
      let timeLineScaleLinearAxisLeft = d3.scaleLinear().range([0, that.chartHeight - that.margin * 2]).domain([that.maxValue, 0]) // 用于绘制体积柱状图的比例尺
      let timeLineScaleLinearAxisRight = d3.scaleLinear().range([0, that.chartHeight - that.margin * 2]).domain([that.maxDis, 0]) // 用于绘制最大距离柱状图的比例尺
      ...
      let timeLineAxisRight = d3.axisRight(timeLineScaleLinearAxisRight).tickSize(2)    // 刻度点
      // let timeLineAxisColorScale = d3.scaleSequential(d3.interpolateRainbow).domain([0, that.clusterBar]) // 填充颜色

      // 坐标轴位置
      that.chart.selectAll("g.axisBottom").data([1]).join("g").attr("transform", `translate(${[5, that.chartHeight - that.margin * 2]})`).attr("class", "axisBottom").call(timeLineAxisBottom)
      that.chart.selectAll("g.axisLeft").data([1]).join("g").attr("class", "axisLeft").call(timeLineAxisLeft).attr("transform", `translate(${[5, 0]})`);
      ...
      // 边框
      that.chart.selectAll("g.timeLine_chart_border").data([1]).join("g").attr("transform", `translate(${[5, that.chartHeight - that.margin * 2]})`).attr("class", "timeLine_chart_border")
      ...
      let rectBarChart = that.chart.selectAll("g.rectBar").data([1]).join("g").attr("transform", `translate(${[0, 0]})`).attr("class", "rectBar")
      // 绘制柱状图
      that.chart.selectAll("rect.bar").data(that.barData).join("rect").attr("class", d => "bar " + "cluster" + that.bar.barClass + " cluster_" + d["j"] + that.bar.barClass)
        .attr("id", d => {
          return "bar_" + d["frame_pocket"]
        })
        .attr("x", d => (d["i"] * that.barWidth + that.barWidth / that.clusterBar * d["j"]) + 5)
        .attr("y", d => that.chartHeight - timeLineScaleLinearAxisLeft(that.maxValue - d["volume"]) - that.margin * 2)
        .attr("width", that.barWidth / that.clusterBar)   // 宽度为
        .attr("height", d => timeLineScaleLinearAxisLeft(that.maxValue - d["volume"]))
      ...
      // 绘制图例
      that.legend = that.chart.selectAll("g." + "cluster" + that.bar.barClass).data(that.frame).join("g").attr("class", (d, i) => {
        return "cluster" + that.bar.barClass + " cluster_" + i + that.bar.barClass
      ...
      let timeLine_dragBehavior = d3.drag().on("drag", function () {
        ...
        if ((x >= 0 && (x + that.margin) <= border) && (x + dx >= 0 && (x + dx + that.margin) <= border)) {
        ...
        that.highlightScatter(that.framePocketStr.split(",")[1].replaceAll("-", ","))
      })
      // 绘制滑块
    ...
    mouseEnterLegend(index) {
      ...
      d3.selectAll(".cluster_" + index + this.bar.barClass).style("opacity", 0.5)
    },
    mouseLeaveLegend() {
