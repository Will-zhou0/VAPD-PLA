<template>
  <div class="com-page">
    <div class="center_center">
      <div class="input-group input_center_center">
        &nbsp;&nbsp;&nbsp;&nbsp;
        <div class="input-group-prepend">
          <span class="input-group-text">Similarity Min:</span>
        </div>

        <input style="width: 60px;" type="number" step="0.01" min="0" max="1" :value="similarMin"
          @change="inputChangeSimilarMin" @blur="inputChangeSimilarMin" />
        <span class="input-group-text">Max:</span>
        <input style="width: 60px;" type="number" step="0.01" min="0" max="1" :value="similarMax"
          @change="inputChangeSimilarMax" @blur="inputChangeSimilarMax" />
        <div class="input-group-prepend">
          <span class="input-group-text">The number of pockets: {{ pocketsNum }}</span>
        </div>
      </div>
      <div class="sub_center_center" draggable="true" @dragstart="scatterSubDragStart">
...
export default {
  ...
  computed: {
    ...mapState(["zoomFactor", "radius", "similarMin", "similarMax", "scatterPlotFramePocketData", "inOrOutDegree"]), // 默认缩放比例
  },
  ...
  // 存储数据
  beforeDestroy() { // 销毁前自动触发

  },
  methods: {
    ...mapMutations(["changZoomFactor", "initScatterPlotFramePocketData", "changeSimilarMin", "changeSimilarMax", "changeInOrOutDegree"]),
    async initRequire() {
      let that = this;
      await request({
        url: "/forcePlot/index",
    ...
    initData() {
      ...
      that.scatterSvg = d3.select(".center_center").append("svg").attr("width", that.width - 2 * that.margin).attr("height", that.height - 2 * that.margin).attr("transform", `translate(${[that.margin, that.margin]})`);
      that.scatterSvg.append("defs").append("clipPath").attr("id", "scatterChart").append("rect").attr("width", that.width - that.margin * 2).attr("height", that.height - that.margin * 2);
      that.scatterChart = that.scatterSvg.append("g").attr("transform", `translate(${[that.margin * 2, 0]})`)
      that.scatterView = that.scatterChart.append("g").attr("class", "scatterView").attr("clip-path", "url(#scatterChart)").append("g");

      that.scatterScaleX = d3.scaleLinear().range([0, that.width - 2 * that.margin]);
      that.scatterScaleY = d3.scaleLinear().range([0, that.height - 2 * that.margin]);   // 刻度尺
    ...
    drawChart() {
      let that = this;
      that.scatterCircles = that.scatterView.selectAll(".scatterDot")
        .data(that.scatterPlotFramePocketData["forceData"])
        .join("circle")
        .attr("id", d => d["frame_pocket"])
        .attr("class", d => {
          return "scatterDot " + " c_" + d["frame_pocket"] + " c_" + d["frame_pocket"] + "color"
        })
      ...
      d3.select("body").on('keydown', function () {
        ...
        if (d3.event.keyCode === 16) {  // shift
          ...
          d3.select(".sub_center_center").style("display", that.scatterShift ? "" : "none")
        }
      }).on("mousemove", function () {
        if (that.scatterCtrlKeyDown) { // 真 进行lasso操作
    ...
    scatterLassoConfig(circles, lassoStatus) {
      ...
      that.scatterLasso = d3.lasso().closePathSelect(true).closePathDistance(100).items(circles).targetArea(that.scatterSvg)

      function lassoStart() {
        // 圈选前先进行初始化
        scatterStartTime = new Date().getTime();
        if (lassoStatus) {
          that.scatterLasso.items()
            .attr("r", that.radius / that.zoomFactor)
      ...
      function lasso_draw() {
        that.scatterLasso.possibleItems().classed("not_possible", false).classed("possible", false);
        that.scatterLasso.notPossibleItems().classed("not_possible", true).classed("possible", false)
      }

      function lasso_end() {
        that.scatterLasso.items().style("fill", d => d["color"]).classed("not_possible", false).classed("possible", false)
        // selected points
        let pockets = []
        that.scatterLasso.selectedItems().classed("selected", true).attr("r", d => {
          pockets.push(d)
        ...
        that.scatterLasso.notSelectedItems().attr("r", that.radius / that.zoomFactor).style("opacity", 0.5);
        // 表示没有选中
      ...
      if (lassoStatus) {
        that.scatterSvg.call(that.scatterLasso.on("start", lassoStart).on("draw", lasso_draw).on("end", lasso_end))
      } else {
        that.scatterSvg.call(that.scatterLasso.on("start", null).on("draw", null).on("end", null))
      }
    },
    scatterZoomConfig(zoomStatus) {
      let that = this;
      let currentZoom = d3.zoomIdentity
      const extent = [[0, 0], [that.width, that.height]]
      ...
      function zooming() {
        ...
        // 散点
        d3.selectAll("circle.scatterDot").attr("r", that.radius / that.zoomFactor).style("opacity", 1)
        // 连线
        d3.select(".scatterView").selectAll("line").attr("stroke-width", 1 / that.zoomFactor)
      }
      ...
      that.scatterZoom = d3.zoom().scaleExtent(that.scaleExtent).extent(extent).translateExtent(extent)
      if (zoomStatus) {
        that.scatterSvg.call(that.scatterZoom.on("zoom", zooming).on("end", zoomEnd))
      } else {
    ...
    subScatterInit(frame_pockets) {
      ...
      // 展示子图数据
      let forcePlotSub_svg = forcePlotSub_div.selectAll("svg.forcePlotSubSvg").data([1]).join("svg").attr("class", "forcePlotSubSvg").attr("width", that.subScatterWidth).attr("height", that.subScatterHeight)
      let forcePlotSub_scaleX = d3.scaleLinear().range([0, that.subScatterWidth]).domain([-that.maxData["maxX"], that.maxData["maxX"]]);
      let forcePlotSub_scaleY = d3.scaleLinear().range([0, that.subScatterHeight]).domain([-that.maxData["maxY"], that.maxData["maxY"]]);   // 刻度尺
      let pockets = []
      forcePlotSub_svg.selectAll("circle.subCircle")
        .data(frame_pockets)
        .join("circle")
    ...
    scatterMouseEnter(node) {
      ...
      that.scatterView.selectAll("text.scatterToopTips").data([1]).join("text")
      ...
      that.scatterView.select(".c_" + node["frame_pocket"]).attr("r", that.radius * 2 / that.zoomFactor)
      that.scatterView.selectAll(".l_" + node["frame_pocket"]).attr("stroke-width", 3 / that.zoomFactor)
    },
    scatterMouseOut(node) {
      let that = this;
      that.scatterView.selectAll(".scatterToopTips").data([]).join("text")
      that.scatterView.select(".c_" + node["frame_pocket"]).attr("r", that.radius / that.zoomFactor)
      that.scatterView.selectAll(".l_" + node["frame_pocket"]).attr("stroke-width", 1 / that.zoomFactor)
    ...
    scatterSubDragStart(event) {
      ...
      event.dataTransfer.setData("framePocket", d3.select(".sub_center_center").attr("frame_pocket"))
    ...
    parallelToScatterHighlight(node) {
      let that = this;
      d3.selectAll("circle.scatterDot").style("opacity", 0.3).attr("r", that.radius / that.zoomFactor)
      d3.select($("#" + node)[0]).attr("r", that.radius * 2 / that.zoomFactor).style("opacity", "1");
    },
    ...
    drawLinkLine(node) {
      ...
      chartView.select("g").selectAll("line").data(node)
        .join("line")
